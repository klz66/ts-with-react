 ...
 * @LastEditTime: 2021-04-17 00:18:02
 */
...
function LikeList(props) {
  ...
  async function getArticleList(id){
    let res = await http.get(`${demoUrl}/blogservice/blog-like/getLikeBlogMember/${id}/${current}/5`);
  ...
  async function getMoreList(current){
    let res = await http.get(`${demoUrl}/blogservice/blog-like/getLikeBlogMember/${props.authorId}/${current}/5`);
          ...
          <List.Item>
            <div style={{display:'flex'}}>
              ...
              <div style={{display:'flex',flexDirection:'column',justifyContent:'space-between',marginLeft:'10px'}}>
                <div> {item.memberNickname} 点赞了你的文章 <span onClick={()=>{window.open('/detail/' + item.blogId)}} style={{color:'blue'}}>{item.blogTitle}</span></div>
                {/* <div style={{fontSize:'12px',color:'#69696'}}>于{item.gmtCreate.slice(0,10)}加入&nbsp;&nbsp;
                  发表了{item.blogNum}篇
                </div> */}
